...
import { CheckCircle2, Gift, Percent, Sparkles, X, type LucideIcon } from "lucide-react";
...
/**
 * Web port of the mobile POS's own DiscountSheet — same picks: a simple
 * catalog rule (Senior/PWD, Employee, custom — capturing an ID when the
 * rule requires one), a loyalty reward the selected customer's points
 * balance clears, and any complex promo whose conditions the cart already
 * meets. All three land as order-level AppliedOrderDiscount entries,
 * separate from the per-line counter-discount price override.
 */
export function DiscountRulesDialog({
  ...
  return (
    <Dialog
      ...
      title="Rules & loyalty"
      description="Senior/PWD and other simple rules, qualifying promos, and loyalty reward redemptions."
    >
      <div className="space-y-5">
        {pendingRule ? (
          <div className="space-y-3 rounded-sm border border-primary-soft bg-primary-tint p-3">
            ...
            <Field label="ID number" required={false}>
              <Input value={idNumber} onChange={(event) => setIdNumber(event.target.value)} placeholder="Senior/PWD ID #" />
            </Field>
          ...
          <>
            ...
            {nothingQualifies ? (
              <p className="py-6 text-center text-body text-ink-muted">Nothing qualifies for this cart right now.</p>
            ) : null}
          </>
        )}
        ...
        {applied.length > 0 ? (
          <div className="space-y-1.5 border-t border-border pt-3">
            <p className="text-caption font-semibold text-ink-muted">APPLIED</p>
            {applied.map((entry) => (
              <div
                key={entry.id}
