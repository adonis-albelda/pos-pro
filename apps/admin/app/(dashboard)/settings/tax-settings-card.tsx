...
export function TaxSettingsCard({ embedded = false }: { embedded?: boolean }) {
  ...
  if (query.isError || !query.data) {
    if (embedded) {
      return <ErrorNote>Could not load tax settings.</ErrorNote>;
    }
    return (
      <Card>
        <CardHeader icon={Percent} title="Tax & discounts" description="VAT registration and promo behaviour." />
  ...
  const body = (
    <div className={embedded ? "space-y-5" : "space-y-5 px-4 py-5 sm:px-6"}>
