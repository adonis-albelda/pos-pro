 ...
 * Laravel side. LoginController forces a token expiring SESSION_DAYS out
 * for such an account regardless of the app's normal session length; the
 * admin web's DemoSessionBanner reads these for display copy. DISABLE_AFTER_DAYS
 * and PURGE_AFTER_DAYS match the lifecycle command that disables then deletes
 * the demo company. The flag itself lives on `User.isDemo`, set by a
...
export interface User {
  ...
  /** Self-service override of store_settings.idleTimeoutMinutes. Null = use the branch default. */
